...
public class UserServlet extends BaseServlet {
	...
	public String login(HttpServletRequest req, HttpServletResponse resp){
		...
		try {
			...
			if (existUser == null) {
				...
				// 用户登录成功
				//第一个bug的解决：第二个用户登陆后要将之前的session销毁！！
				req.getSession().invalidate();
				
				//第二个bug的解决：判断用户是否已经存在Map集合中，若存在；已经在列表中，销毁其session
				...
				//使用监听器：HttpSessionBindingListener,作用在JavaBean上的
	 ...
	 * @throws Exception 
	 */
	public String kick(HttpServletRequest req, HttpServletResponse resp) throws Exception{
		//1.接收参数
		...
		Map<User, HttpSession> userMap = (Map<User, HttpSession>) getServletContext()
				.getAttribute("userMap");
		//获得这个用户所对应的session，如何知道是哪个用户呢？id已经传递过来，去数据库中查询。
		//已经重写了User的equals和hashCode方法，那么只要用户的id相同就认为是同一个用户。
		User user = new User();
		user.setId(id);
